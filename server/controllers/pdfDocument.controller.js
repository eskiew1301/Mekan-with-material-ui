...
exports.addPdfDocument = async (req, res, next) => {
  if (!req.file) {
  ...
  try {
    ...
    res.status(201).json({
      ...
      documentUrl: `${req.protocol}://${req.get("host")}/uploads/pdfDocument/${req.file.filename}`,
    });
  } catch (err) {
...
exports.getAllPdfDocuments = async (req, res) => {
  try {
    ...
    const error = new HttpError("Fetching document failed, please try again later.", 500);
    return next(error);
  }
};

exports.getOnePdfDocument = async (req, res, next) => {
